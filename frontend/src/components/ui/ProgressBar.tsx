    ...
    <div className={cn('flex items-center gap-3', className)}>
      ...
      >
        <motion.div
          ...
          className="h-full rounded-full bg-accent-gradient shadow-[0_0_10px_rgba(14,165,233,0.5)]"
        />
      </div>
      {label !== undefined && (
        <span className="w-10 shrink-0 text-right text-xs font-semibold tabular-nums text-accent-300">
          {Math.round(clamped)}%
        </span>
      )}
    </div>
