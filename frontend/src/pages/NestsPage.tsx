  ...
  if (nests.length === 0) {
    return (
      <motion.div variants={cardStagger} initial="hidden" animate="show" className="space-y-6">
        <PageHeader
          title="My Nest"
          description="Your small curated groups — where your people live."
        />
        <NestEmptyState />
      </motion.div>
    );
  ...
  const renderCard = (nest: NestResponse) => {
    ...
    return (
      <motion.button
        ...
        className="group relative overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--grad-card)] p-6 text-left backdrop-blur-xl transition-shadow duration-300 hover:shadow-[var(--shadow-lg)]"
      >
        <div
          className="pointer-events-none absolute inset-x-6 top-0 h-px bg-gradient-to-r from-transparent via-[var(--accent-400)]/50 to-transparent"
          aria-hidden="true"
        />
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate font-['Space_Grotesk'] text-lg font-bold text-[var(--text-primary)] transition-colors group-hover:text-[var(--accent-300)]">
              {nest.name}
            </h3>
            <p className="mt-1 flex items-center gap-1 text-xs text-[var(--text-muted)]">
              <MapPin className="h-3 w-3 text-[var(--accent-400)]" aria-hidden="true" />
              {nest.city}
            </p>
          </div>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-[var(--color-border)] pt-4">
          <span className="inline-flex items-center gap-1.5 text-xs text-[var(--text-secondary)]">
            <Users className="h-3.5 w-3.5 text-[var(--accent-400)]" aria-hidden="true" />
            {memberCount} member{memberCount === 1 ? '' : 's'}
          </span>
          {nest.startDate && (
            <span className="inline-flex items-center gap-1.5 text-xs text-[var(--text-secondary)]">
              <CalendarDays className="h-3.5 w-3.5 text-[var(--accent-400)]" aria-hidden="true" />
              Since {formatDate(nest.startDate)}
            </span>
          )}
          <ArrowRight
            className="h-4 w-4 text-[var(--accent-400)] transition-transform duration-200 group-hover:translate-x-1"
            aria-hidden="true"
          />
        </div>
      </motion.button>
