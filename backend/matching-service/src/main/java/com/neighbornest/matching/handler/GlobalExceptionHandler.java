package com.neighbornest.matching.handler;
...
public class GlobalExceptionHandler {
    ...
    /**
     * Handles {@link ProposalExpiredException} — returns 400 BAD REQUEST.
     */
    @ExceptionHandler(ProposalExpiredException.class)
    public ResponseEntity<ErrorResponse> handleProposalExpired(
            final ProposalExpiredException ex, final HttpServletRequest request) {
        log.warn("Proposal expired: {}", ex.getMessage());
        return buildErrorResponse(HttpStatus.BAD_REQUEST, ex.getMessage(), request);
    }
