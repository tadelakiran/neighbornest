...
import com.neighbornest.chatservice.dto.response.ErrorResponse;
 ...
 * with appropriate HTTP status codes. Note that exceptions raised inside
 * {@code @MessageMapping} WebSocket handlers are handled by the STOMP layer
 * (which sends an ERROR frame to the client), not by this handler.
 * </p>
 *
...
public class GlobalExceptionHandler {
    ...
    /**
     * Handles {@link org.springframework.dao.DataIntegrityViolationException}
     * — returns 409 CONFLICT for duplicate-key violations (e.g. two users
     * racing to create the same conversation, or a duplicated read receipt),
     * otherwise 500.
     * <p>
     * Two concurrent {@code POST /api/chat/dm/start} calls can both pass the
     * {@code findByParticipant1IdAndParticipant2Id} guard and collide on the
     * unique conversation constraint; the DB constraint is the final arbiter
     * and must surface as a 409 so the client can simply retry the request.
     * </p>
     */
    @ExceptionHandler(org.springframework.dao.DataIntegrityViolationException.class)
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(
            final org.springframework.dao.DataIntegrityViolationException ex, final HttpServletRequest request) {
        ...
        if (root instanceof final java.sql.SQLIntegrityConstraintViolationException constraintViolation
            ...
            log.warn("Duplicate key violation: {}", root.getMessage());
            return buildErrorResponse(HttpStatus.CONFLICT,
                    "That resource already exists. Try again to fetch it.", request);
        }
    ...
    /**
     * Handles {@link UnauthorizedException} — returns 401 UNAUTHORIZED.
     */
    ...
    /**
     * Handles {@link ServiceUnavailableException} — returns 503 SERVICE UNAVAILABLE.
     */
    @ExceptionHandler(ServiceUnavailableException.class)
    public ResponseEntity<ErrorResponse> handleServiceUnavailable(
            final ServiceUnavailableException ex, final HttpServletRequest request) {
