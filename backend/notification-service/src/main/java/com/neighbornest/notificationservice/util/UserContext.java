package com.neighbornest.notificationservice.util;
 ...
 *       Authorization header — the same convention used by the other domain
 *       services.</li>
 * </ol>
 * </p>
 *
...
public class UserContext {
    ...
    public Long requireProfileId() {
        final UserProfileResponse profile = requireProfile();
        return profile.getId();
    }

    /**
     * Resolves the current caller's full profile.
     *
     * @return the caller's profile
     * @throws BadRequestException if the profile cannot be resolved
     */
    public UserProfileResponse requireProfile() {
        final Optional<Long> headerId = profileIdFromHeader();
        if (headerId.isPresent()) {
            final UserProfileResponse fromHeader = UserProfileResponse.builder().id(headerId.get()).build();
            return fromHeader;
        }

        final UserProfileResponse profile = userServiceClient.getMyProfile();
        if (profile == null || profile.getId() == null) {
            log.warn("Could not resolve profile for the current caller");
